...
import ch.hsr.ifs.templator.plugin.util.ImageCache;
import ch.hsr.ifs.templator.plugin.util.ImageCache.ImageID;
...
public class SearchBar {
	...
	private void scrollToRegion(int offset, int length) {
		...
		Rectangle viewBounds = new Rectangle(scrolledComposite.getOrigin().x, scrolledComposite.getOrigin().y,
				scrolledComposite.getClientArea().width, scrolledComposite.getClientArea().height);
	...
	private Composite createSerachBar() {
	...
	public void show() {
		if (searchComposite == null) {
			searchComposite = createSerachBar();
