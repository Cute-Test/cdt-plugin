...
public final class ClassTemplateMemberResolver {
	...
	public static ICPPSpecialization resolveClassTemplateMember(UnresolvedNameInfo memberName,
		...
		} else if (type == NameTypeKind.DEFERRED_MEMBER_CLASS_INSTANCE) {
		} else if (type == NameTypeKind.UNKNOWN_MEMBER_CLASS) {

		} else if (type == NameTypeKind.DEFERRED_MEMBER_CLASS_INSTANCE) {
		} else if (type == NameTypeKind.UNKNOWN_MEMBER_ALIAS_TEMPLATE_INSTANCE) {
		}

		return finalResolvedMember;
	}

	public static ICPPSpecialization resolveMemberAliasTemplateInstance(UnresolvedNameInfo aliasunresolvedName,
			AbstractResolvedNameInfo parent) throws TemplatorException {
	...
	public static ICPPClassSpecialization resolveOwner(UnresolvedNameInfo memberunresolvedName,
			AbstractResolvedNameInfo parent) throws TemplatorException {
		...
		if (owner instanceof ICPPTemplateParameter) {
			...
			if (definitionName != null) {
				definitionName = analyzer.extractResolvingName(definitionName, true).getTypeName();
				AbstractResolvedNameInfo ownerInfo = analyzer.getTypeDeducer().createNewContext(definitionName, parent,
						true);
				if (ownerInfo != null) {
	...
	public static ICPPSpecialization resolveClassTemplateMemberFunction(UnresolvedNameInfo unresolvedName,
			ICPPClassSpecialization resolvedOwner, AbstractResolvedNameInfo parent) throws TemplatorException {
		IASTName resolvingName = unresolvedName.getResolvingName();
		IBinding originalBinding = unresolvedName.getBinding();

		if (resolvedOwner != null) {
			IBinding specializedMemberFunction = ClassTemplateMemberResolver.instantiateMember(originalBinding,
					resolvedOwner, resolvingName, parent);
			if (specializedMemberFunction instanceof ICPPSpecialization
					&& specializedMemberFunction instanceof ICPPMethod) {
	...
	public static IBinding instantiateMember(IBinding deferredMemberBinding, ICPPClassSpecialization ownerInstance,
			IASTName point, AbstractResolvedNameInfo parent) throws TemplatorException {
		...
		if (specializedMember instanceof CPPFunctionSet) {
			...
			specializedMember = FunctionCallResolver.resolveDeferredCall(point, candidateBindings, ownerInstance,
					parent, parent.getAnalyzer());
		}
