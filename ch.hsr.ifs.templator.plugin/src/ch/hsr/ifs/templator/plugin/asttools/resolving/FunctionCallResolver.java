...
public final class FunctionCallResolver {
	...
	public static IFunction resolveCall(ICPPASTFunctionCallExpression functionCall,
		...
		if (binding instanceof IFunction) {
			...
			throw new TemplatorException(
					functionCall.getRawSignature() + " resolved to " + binding.getClass() + " instead of a function.");
		}
	}
	 ...
	 */
	private static ICPPFunction[] removeWrongFunctionTypeCandidates(IASTName functionCallName,
			ICPPFunction[] originalCandidates) {
	...
	public static IBinding resolveDeferredCall(IASTName functionCallName, ICPPFunction[] candidates,
			IType impliedObjectType, AbstractResolvedNameInfo parentInstance, ASTAnalyzer analyzer)
		...
		if (templateArgumentMap != null) {
			LookupData templateContextData = TemplateContextLookupData.createLookupData(
					ASTTools.extractTemplateInstanceName(functionCallName), templateArgumentMap, impliedObjectType);
			try {
				final IBinding resolvedFunction = CPPSemantics.resolveFunction(templateContextData, candidates, true);
			...
			throw new TemplatorException(
					functionCallName + " could not be resolved because the template arguments could not be deduced.");
		}
