package ch.hsr.ifs.templator.plugin.asttools.formatting;
...
import ch.hsr.ifs.templator.plugin.asttools.FindFunctionCallExpressionsVisitor;
import ch.hsr.ifs.templator.plugin.asttools.data.AbstractResolvedNameInfo;
import ch.hsr.ifs.templator.plugin.asttools.data.CPPASTTemplateIdPreservingOriginalNode;
import ch.hsr.ifs.templator.plugin.asttools.data.FunctionCall;
import ch.hsr.ifs.templator.plugin.asttools.data.MemberFunctionInstance;
import ch.hsr.ifs.templator.plugin.asttools.data.ResolvedName;
import ch.hsr.ifs.templator.plugin.asttools.data.TemplateInstance;
...
public final class ASTTemplateFormatter {
	...
	static {
		try {
			getQualifiedNameMethod = ReflectionMethodHelper.getNonAccessibleMethod(ToggleNodeHelper.class,
					"getQualifiedName", IASTFunctionDeclarator.class, IASTNode.class);
		} catch (Exception e) {
	...
	public static void format(AbstractResolvedNameInfo resolvedName) throws TemplatorException {
		...
		if (resolvedName instanceof TemplateInstance) {
			...
			formatTemplateDeclarator((ICPPASTTemplateDeclaration) definitionCopy,
					((TemplateInstance) resolvedName).getTemplateArgumentMap());
		} else if (resolvedName instanceof FunctionCall || resolvedName instanceof MemberFunctionInstance) {
	...
	private static void formatMethodName(IASTDeclaration definitionCopy, AbstractResolvedNameInfo member)
		...
		if (oldDeclaration instanceof ICPPASTFunctionDefinition) {
			...
			if (!(oldDeclaratorName instanceof ICPPASTQualifiedName)) {
				ICPPASTTemplateDeclaration classTemplateDeclaration = ((TemplateInstance) member.getParent())
						.getDefinition();
				try {
					ICPPASTQualifiedName newDeclaratorName = ReflectionMethodHelper
							.<ICPPASTQualifiedName> invokeStaticMethod(getQualifiedNameMethod,
					...
					throw new TemplatorException(
							"Failed to get qualified name for a member function. The class name is thus not shown formatted for "
									+ declarator.getName() + ".",
							e);
				}
			}
	...
	private static void formatTemplateDeclarator(ICPPASTTemplateDeclaration templateCopy,
			TemplateArgumentMap templateArgumentMap) throws TemplatorException {
	...
	private static void formatBody(IASTDeclaration definitionCopy, AbstractResolvedNameInfo resolvedName)
			throws TemplatorException {
		...
		for (ResolvedName subResolvedNameInfo : resolvedName.getSubNames()) {
			IASTName subResolvedName = subResolvedNameInfo.getOriginalName();
			ICPPASTFunctionCallExpression subCall = ASTTools.findFirstAncestorByType(subResolvedName,
					ICPPASTFunctionCallExpression.class, 5);
			if (subCall != null) {
	...
	private static void formatTemplateParameters(ICPPASTTemplateParameter[] parameters,
			TemplateArgumentMap templateArgumentMap, ICPPNodeFactory factory) throws TemplatorException {
		for (ICPPASTTemplateParameter parameter : parameters) {
			if (parameter.isParameterPack()) {
				...
				// throw new TemplatorException("Variadic templates are not yet supported.");
			} else {
				...
				if (argument != null) {
					...
					// nested template parameters as V in template<template<typename T> typename V> void foo(...)
						...
						addArgument((ICPPASTTemplatedTypeTemplateParameter) parameter, argumentString,
								templateArgumentMap, factory);
					}
				}
			}
		}
	...
	public static ICPPASTFunctionCallExpression formatCall(ICPPASTFunctionCallExpression call,
			ICPPTemplateArgument[] templateArgs, boolean copy) {
		...
		if (functionNameExpression instanceof ICPPASTFieldReference) {
			...
			ICPPASTName formattedFunctionLastName = addTemplateArgsToName(fullyQualifiedFunctionName.getLastName(),
					templateArgs);
	...
	private static void addArgument(ICPPASTTemplatedTypeTemplateParameter parameter, String argumentString,
			TemplateArgumentMap templateArgumentMap, ICPPNodeFactory factory) throws TemplatorException {
	...
	private static void addArgument(ICPPASTSimpleTypeTemplateParameter parameter, String argumentString,
			ICPPNodeFactory factory) {
		ICPPASTNamedTypeSpecifier nameSpecifier = factory
				.newTypedefNameSpecifier(factory.newName(argumentString.toCharArray()));
	...
	private static void addArgument(ICPPASTParameterDeclaration parameter, String argumentString,
			ICPPNodeFactory factory) {
