package ch.hsr.ifs.cute.templator.plugin.asttools.resolving.nametype;
...
public class NameTypeDeducer {
	...
	public TypeNameToType getTypeFromDefinition(IASTName definitionName, AbstractResolvedNameInfo parent)
	...
	public TypeNameToType getType(IASTName originalName, ITypedef alias, AbstractResolvedNameInfo parent)
			throws TemplatorException {
	...
	public TypeNameToType resolveTypeName(TypeNameToType typeNameToBinding, AbstractResolvedNameInfo parent)
			throws TemplatorException {
		if (!typeNameToBinding.isCompletelyResolved()) {
			...
			if (unresolvedNameInfo.isRelevant() && !unresolvedNameInfo.getType().isReplacementNeeded()) {
				AbstractResolvedNameInfo newParentName = AbstractResolvedNameInfo.create(unresolvedNameInfo, parent,
						analyzer);
	...
	public TypeNameToType getType(ICPPASTQualifiedName qualifiedName, AbstractResolvedNameInfo parent)
			throws TemplatorException {

		ICPPASTNameSpecifier[] qualifierNameSegments = qualifiedName.getAllSegments();
		QualifiedNameSegment currentSegment = null;
		for (int i = 0; i < qualifierNameSegments.length; i++) {
			ICPPASTNameSpecifier nameSegmentSpecifier = qualifierNameSegments[i];

			if (!(nameSegmentSpecifier instanceof IASTName)) {
				throw new TemplatorException("Could not resolve " + nameSegmentSpecifier.getRawSignature()
						+ " inside the qualified name " + qualifiedName.getRawSignature());
			}
	...
	public TypeNameToType getMemberType(IBinding memberBinding, TypeNameToType ownerType,
	...
	public AbstractResolvedNameInfo createNewContext(IASTName resolvingName, AbstractResolvedNameInfo parent,
	...
	public AbstractResolvedNameInfo createNewContext(IASTName resolvingName, IBinding binding,
			AbstractResolvedNameInfo parent, boolean resolve) throws TemplatorException {
