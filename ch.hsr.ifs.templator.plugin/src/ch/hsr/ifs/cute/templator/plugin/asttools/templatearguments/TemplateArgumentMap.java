package ch.hsr.ifs.cute.templator.plugin.asttools.templatearguments;
...
public class TemplateArgumentMap extends CPPTemplateParameterMap {
	...
	private static ICPPTemplateParameter getParameterBinding(ICPPASTTemplateParameter astParameter) {
		...
		if (astParameter instanceof ICPPASTParameterDeclaration) {
			ICPPASTParameterDeclaration parameterDeclaration = (ICPPASTParameterDeclaration) astParameter
					.getOriginalNode();
			...
			ICPPASTSimpleTypeTemplateParameter typeParameter = (ICPPASTSimpleTypeTemplateParameter) astParameter
					.getOriginalNode();
			paramName = typeParameter.getName();
		} else if (astParameter instanceof ICPPASTTemplatedTypeTemplateParameter) {
			ICPPASTTemplatedTypeTemplateParameter typeParameter = (ICPPASTTemplatedTypeTemplateParameter) astParameter
					.getOriginalNode();
			paramName = typeParameter.getName();
		} else {
			TemplatorLogger.logError(
					"ICPPASTTemplateParameter was of unexpected type " + astParameter.getClass().toString(), null);
			return null;
		}
