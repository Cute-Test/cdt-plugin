...
Unit testing suports code quality and is a corner stone of agile software development. CUTE and its 
Eclipse plug-in are a easy to use testing framework for C++.
...
given during construction using the CUTE() macro or derived from the function's typeid.

Running a single test is not very usefull. But having a larger collection larger collection of test cases an 
...
%TODO Codebeispiel aus aktuellem CUTE-plug-in
\begin{verbatim}
void runSuite(){
  cute::suite s;
...
The plug-in uses the standard cute runner with specical output listener that writes the test results to 
\verb|std::cout| using a defined format. This output is parsed and a internal model of the test and there 
...
CUTE's result view provieds all the important features Java developer know from the JUnit-plug-in:
\begin{itemize}
 \item Red / Green Bar
 \item test navigator
...
\begin{center}
 \includegraphics[bb=0 0 481 185]{results.png}
 % results.png: 481x185 pixel, 72dpi, 16.97x6.53 cm, bb=0 0 481 185
\end{center}
...
library that are part of the proposal C++ standard extension \verb|std::tr1|.
