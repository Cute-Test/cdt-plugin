...
public class SingleSelectionChangeAction implements ConfigChangeAction {
     ...
     */
    public SingleSelectionChangeAction(IASTNode originalParent,
            IASTNode copyParent, IASTNode originalSubNode) {
     ...
     */
    private boolean traverseRecursive(IASTNode origParent, IASTNode copyParent,
        ...
        for (int i = 0; i < origParent.getChildren().length; ++i) {
            if (traverseRecursive(origParent.getChildren()[i],
                    copyParent.getChildren()[i], config))
                return true;
        }
        return false;
    }
     ...
     *            Config.
     */
    private void disableActionExceptNodeMatch(IASTNode node,
            TransformConfiguration config) {
