...
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.configuration.action.FunctionOrderingChangeAction;
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.util.TypeInformation;
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.util.referencelookup.FunctionNormalReferenceLookupStrategy;
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.util.referencelookup.FunctionSpecializedReferenceLookupStrategy;
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.util.referencelookup.ReferenceLookupStrategy;
...
public class ETTPFunctionTransform extends Transform {
    ...
    private IASTName getFunctionName() {
        return ((ICPPASTFunctionDefinition) getOriginalNode()).getDeclarator()
                .getName();
    }
    ...
    private List<TypeInformation> getTypesToIncludeInCall() {
        ...
        List<TypeInformation> typesToIncludeInCall = new ArrayList<TypeInformation>();
     ...
     *            Modification collector.
     */
    private void adjustCall(List<TypeInformation> types,
            ICPPASTFunctionCallExpression call, ModificationCollector collector) {
        CPPASTTemplateId newFunctionName = new CPPASTTemplateId(
                getFunctionName().copy());
        for (TypeInformation type : types) {
            newFunctionName.addTemplateArgument(createTypeId(type));
        }
        IASTIdExpression newCallExpr = (CPPASTIdExpression) call
                .getFunctionNameExpression().copy();
        newCallExpr.setName(newFunctionName);
        createRewriter(collector, getUnitOf(call)).replace(
                call.getFunctionNameExpression(), newCallExpr,
                createCallEditText());
    }
     ...
     *            Function call.
     */
    private void resolveMissingTypes(List<TypeInformation> types,
            ICPPASTFunctionCallExpression call) {
        for (TypeInformation type : types) {
     ...
     *            Function call.
     */
    private void resolveMissingType(TypeInformation type,
            ICPPASTFunctionCallExpression call) {
        if (!isTemplateCall(call)) {
            resolveByParameters(type, call);
            return;
        }
        ICPPASTTemplateParameter[] templParams = getTemplateDeclaration()
        ...
        if (templateArgumentPos > (funcCallExpr.getTemplateArguments().length - 1)) {
            ...
            CPPASTTypeId templateArg = (CPPASTTypeId) funcCallExpr
                    .getTemplateArguments()[templateArgumentPos];
    ...
    private CPPASTTemplateId getTemplateId(ICPPASTFunctionCallExpression call) {
        return (CPPASTTemplateId) ((CPPASTIdExpression) call
     ...
     *            Function call.
     */
    private void resolveByParameters(TypeInformation type,
            ICPPASTFunctionCallExpression call) {
        int paramPos = 0;
    ...
    private CPPASTFunctionDeclarator getFunctionDeclarator() {
        return (CPPASTFunctionDeclarator) ((ICPPASTFunctionDefinition) getOriginalNode())
                .getDeclarator();
    }
    ...
    private IBinding getBinding(ICPPASTTemplateParameter param) {
        return ((CPPASTSimpleTypeTemplateParameter) param).getName()
                .getBinding();
    }
    ...
    private boolean isTemplateCall(ICPPASTFunctionCallExpression call) {
        CPPASTIdExpression functionName = (CPPASTIdExpression) call
