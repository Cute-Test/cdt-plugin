package ch.hsr.ifs.cute.refactoringPreview.clonewar.app.view;
...
import ch.hsr.ifs.cute.refactoringPreview.clonewar.app.view.util.TypePair;
...
public abstract class ETTPSelectionWizardPage extends AbstractETTPWizardPage {
    ...
    protected void updateOrderPreview(Table table) {
        ...
        for (TableItem item : table.getItems()) {
            ...
            if (action.shouldPerform()
                    && !types.contains(type.getTemplateName() + COMMA_SPACE))
    ...
    private Font createOrderingTitleFont(Label orderingTitle) {
        return new Font(Display.getCurrent(),
                createOrderingTitleFontData(orderingTitle));
    }
     ...
     *            Transform action.
     */
    protected abstract void createTableItem(Table table, TypeInformation type,
    ...
    private void addInformationLabel() {
        new Label(getPage(), SWT.NONE)
                .setText(Messages.EXTRACTION_PAGE_INFO_MSG);
    }
    ...
    private void changeOrdering(Table table) {
        ...
        for (int i = 0; i < items.length; ++i)
            ((TypePair) items[i].getData()).getTypeInfo().setOrderId(-1);
        for (int i = 0; i < items.length; ++i)
    ...
    private class CellSelectionListener extends MouseAdapter {
        ...
        private String[] lookupProposals() {
            List<String> proposals = new ArrayList<String>(getConfig()
                    .getAllTypes().size());
            for (TypeInformation type : getConfig().getAllTypesOrdered()) {
    ...
    private class SaveConfigurationListener implements Listener {
        ...
        private TypeInformation copy(TypeInformation typeInfo) {
            TypeInformation newTypeInfo = new TypeInformation(
                    typeInfo.getType());
         ...
         *            Variable name.
         */
        private void checkConversionProblems(IType type, String templateName,
                String varName) {
            for (TypeInformation typeInfo : getConfig().getAllTypes()) {
                if (typeInfo.getTemplateName().equals(templateName)) {
                    if (!(CPPVisitor.isSameType(typeInfo.getType(), type))) {
                        setMessage(
                                createConversionWarning(templateName, varName),
         ...
         * @return Warning text.
         */
        private String createConversionWarning(String templateName,
                String varName) {
            return Messages.TYPE_CONVERSION_WARNING.replace(FIRST_REPLACE,
                    templateName).replace(SECOND_REPLACE, varName);
        }
    }
