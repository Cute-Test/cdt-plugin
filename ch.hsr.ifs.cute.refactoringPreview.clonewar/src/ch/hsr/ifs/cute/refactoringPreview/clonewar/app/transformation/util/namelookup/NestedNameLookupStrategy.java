package ch.hsr.ifs.cute.refactoringPreview.clonewar.app.transformation.util.namelookup;
...
public class NestedNameLookupStrategy implements NameLookupStrategy {
     ...
     */
    static {
        nameLookupRegistry_.put(CPPASTTemplateId.class,
                new TemplateNameLookupStrategy());
        nameLookupRegistry_.put(CPPASTCastExpression.class,
                new CastNameLookupStrategy());
    }
    ...
    public String lookupName(IASTNode node) {
        ...
        if (!nameLookupRegistry_.containsKey(lookupNode.getClass()))
            return UNRESOLVABLE_NAME;
        String name = nameLookupRegistry_.get(lookupNode.getClass())
                .lookupName(node);
        if (name == null)
            return UNRESOLVABLE_NAME;
        return name;
