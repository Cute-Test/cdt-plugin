...
public class ExistingTemplateChangeAction extends TemplateChangeAction {
    ...
    public void applyChange(TransformConfiguration config,
        ...
        for (ICPPASTTemplateParameter param : getTemplateParameter()) {
            for (TypeInformation type : config.getAllTypes()) {
                if (hasSameType(type, param))
                    setExistingName(type, param, config);
     ...
     *            Config to change.
     */
    private void setExistingName(TypeInformation type,
            ICPPASTTemplateParameter param, TransformConfiguration config) {
        ICPPASTTemplateParameter[] paramArray = new ICPPASTTemplateParameter[] { param };
        updateName(type, config,
                ASTStringUtil.getTemplateParameterArray(paramArray)[0]);
    }
     ...
     */
    private void tryResolveDefaultType(TypeInformation type,
            ICPPASTTemplateParameter param, TransformConfiguration config,
            RefactoringStatus status) {
        if (!isTemplateType(type))
            type.setDefaultType(getDefaultTypeOf(type, config, status));
        else
            type.setDefaultType(getDefaultTypeOf(param));
    }
    ...
    private IASTDeclSpecifier getDefaultTypeOf(ICPPASTTemplateParameter param) {
        IASTTypeId defaultType = ((ICPPASTSimpleTypeTemplateParameter) param)
                .getDefaultType();
        if (defaultType == null)
            return null;
     ...
     */
    private boolean hasSameType(TypeInformation type,
            ICPPASTTemplateParameter param) {
        CPPTemplateDefinition templDefBinding = createTypeBinding(type
                .getType());
        if (templDefBinding == null)
            return false;
        IBinding templBinding = createTemplateBinding(param);
        if (new ASTTypeMatcher().isEquivalent(templBinding, type.getType()))
            return true;
        return false;
    }
    ...
    private IBinding createTemplateBinding(ICPPASTTemplateParameter param) {
        return CPPVisitor
                .createBinding(((CPPASTSimpleTypeTemplateParameter) param)
                        .getName());
    }
    ...
    private IASTName findTypeDefinition(IType type) {
        if (type instanceof CPPTemplateTypeParameter)
            return (IASTName) ((CPPTemplateTypeParameter) type).getDefinition();
        return null;
    }
