...
public class TemplateNameLookupStrategy implements NameLookupStrategy {
    ...
    public String lookupName(IASTNode node) {
        if (isParameter(node))
            return TEMPLATE_SPECIFIER + lookupFromParameter(node);
        if (isBody(node))
            return TEMPLATE_SPECIFIER + lookupFromBody(node);
        if (isReturn(node))
            return TEMPLATE_SPECIFIER
    ...
    private boolean isParameter(IASTNode node) {
        if (findParameterNode(node) == null)
            return false;
        return true;
    }
