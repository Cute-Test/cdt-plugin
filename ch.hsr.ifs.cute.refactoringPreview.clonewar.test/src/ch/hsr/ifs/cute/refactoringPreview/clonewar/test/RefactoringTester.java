...
public class RefactoringTester extends TestSuite{
	...
	/**
	 * Version to add strategies to the tests. Only works with {@link AbstractRefactoringTest}.
	 ...
	 */
	public static Test suite(String name, String file, ConditionCheckStrategy initCheck, ConditionCheckStrategy finalCheck, TestConfigurationStrategy testStrategy, Class<?> expectedException)throws Exception {
		BufferedReader in = createReader(file);
		ArrayList<RefactoringBaseTest> testCases = createTests(in);
		for(RefactoringBaseTest test : testCases) {
			AbstractRefactoringTest abstractTest = (AbstractRefactoringTest)test;
			abstractTest.setInitialCheckStrategy(initCheck);
			abstractTest.setFinalCheckStrategy(finalCheck);
			if(testStrategy!=null)
				abstractTest.setTestStrategy(testStrategy);
			abstractTest.setExceptionClass(expectedException);
		}
		in.close();
