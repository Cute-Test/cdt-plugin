				...
				if(aliasTypeIsFromSameScopeAsAliasDefinitionAndReplacementIsQualified(orignalname, theNameOfTheAlias,theNodeToReplace)){
						...
						if (binding!=null){ // only when alias is actually from a a scope
							...
							if (ntspec.getName() instanceof ICPPASTQualifiedName){
								...
								qname.addName((ICPPASTName) ntspec.getName());
								ntspec.setName(qname);
							}
						}
					}
...
//				specifierFromAlias = factory.newTypedefNameSpecifier(qname);
	...
	private boolean aliasTypeIsFromSameScopeAsAliasDefinitionAndReplacementIsQualified(IASTName orignalname,
			IASTName theNameOfTheAlias2, IASTNode theNodeToReplace2) {
	...
	private ICPPASTFunctionDefinition createFunctionDefinition(CPPASTFunctionDefinition toReplace,
			IASTDeclSpecifier specifierFromAlias, IASTDeclarator declaratorFromAlias) {
	...
	private ICPPASTParameterDeclaration createParameterDeclaration(CPPASTParameterDeclaration toReplace,
			IASTDeclSpecifier specifierFromAlias, IASTDeclarator declaratorFromAlias) {
		IASTDeclSpecifier replacedDeclspec = toReplace.getDeclSpecifier();
		declaratorFromAlias=cloneSpecifierInfo(specifierFromAlias,replacedDeclspec,declaratorFromAlias);
	...
	private IASTSimpleDeclaration createSimpleDeclaration(CPPASTSimpleDeclaration toReplace,
			IASTDeclSpecifier specifierFromAlias, IASTDeclarator declaratorFromAlias) {
	...
	private IASTDeclarator cloneSpecifierInfo(IASTDeclSpecifier target, IASTDeclSpecifier origin, IASTDeclarator declarator) {
		...
		if (origin.getAlignmentSpecifiers() != null && origin.getAlignmentSpecifiers().length>0){
			final int size = origin.getAlignmentSpecifiers().length;
			IASTAlignmentSpecifier[] newalignment = new IASTAlignmentSpecifier[size];
			for(int i = 0; i < size; ++i){
				newalignment[i] = origin.getAlignmentSpecifiers()[i].copy(CopyStyle.withLocations);
			}
			target.setAlignmentSpecifiers(newalignment);
		}
		if (target.isConst() && origin.isConst()){
	...
	private IASTDeclarator handleConstVolatile(IASTDeclSpecifier target, IASTDeclSpecifier origin, IASTDeclarator declarator) {
		...
		if (origin.isConst()||origin.isVolatile()){
			...
			if (inner.getPointerOperators() != null && inner.getPointerOperators().length>0){
				...
				while (i > 0 && (inner.getPointerOperators()[i]==null || inner.getPointerOperators()[i] instanceof ICPPASTReferenceOperator)){--i;}
	...
	private IASTDeclarator createDeclarator(IASTDeclarator theNewDeclarator, IASTDeclarator olddeclar) {
		...
		if (olddeclar != null){
			retval = (IASTDeclarator) olddeclar.copy(CopyStyle.withLocations);
...
//			}
	...
	public void transferDeclaratorExtras(IASTDeclarator target, IASTDeclarator fromtypealias,
			IASTDeclarator toreplace) {
