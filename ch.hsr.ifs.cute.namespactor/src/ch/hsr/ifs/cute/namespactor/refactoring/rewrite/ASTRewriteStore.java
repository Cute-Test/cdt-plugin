package ch.hsr.ifs.cute.namespactor.refactoring.rewrite;
...
/**
 * Rewrite store to register changes for an AST. The class stores all rewrites
 * created during a refactoring process and selects the appropriate one for the
 * current change, by considering the rewrite root of a change. If a change
 * creates a new rewrite, it is saved inside the store for further usage.
 * 
...
public class ASTRewriteStore {
    ...
    private final Map<IASTNode, ASTRewrite> rewrites = new HashMap<IASTNode, ASTRewrite>();
    private final List<IASTChange> changes = new ArrayList<IASTChange>();
     ...
     *            Insertion point.
     */
    public void addInsertChange(IASTNode root, IASTNode newNode,
