...
public class DeclarationHelper {
	...
	public static boolean isPlainCPPASTDeclarator(IASTDeclarator declarator){
		return (declarator instanceof CPPASTDeclarator) 
				&& !(declarator instanceof CPPASTArrayDeclarator)
				&& !(declarator instanceof CPPASTFieldDeclarator) 
				&& !(declarator instanceof CPPASTFunctionDeclarator);
	}
	...
	private static IASTDeclarator combineArrayDeclarators(IASTDeclarator parentdeclarator,
			IASTDeclarator current) {
	...
	private static boolean hasNoPointers(IASTDeclarator current) {
		boolean result = current.getPointerOperators() == null || (current.getPointerOperators().length == 0 || current.getPointerOperators()[0]==null);
		return result;
	}

	private static IASTDeclarator moveNestedNameToParent(IASTDeclarator parentdeclarator,
			IASTDeclarator current) {
