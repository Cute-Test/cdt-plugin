...
import ch.hsr.ifs.cute.core.ui.TestRunnerViewPart;
...
public class ShowResultView extends UIJob{
	...
	private TestRunnerViewPart showTestRunnerViewPartInActivePage(TestRunnerViewPart testRunner) {
		...
		try {
			if (testRunner != null && testRunner.isCreated())
				return testRunner;
			page= CuteCorePlugin.getActivePage();
			if (page == null)
				return null;
	...
	private TestRunnerViewPart findTestRunnerViewPartInActivePage() {
		IWorkbenchPage page= CuteCorePlugin.getActivePage();
		if (page == null)
			return null;
	...
	public IStatus runInUIThread(IProgressMonitor monitor) {
		if (showTestRunnerViewPartInActivePage(findTestRunnerViewPartInActivePage()) == null) {
			return new Status(IStatus.WARNING, CuteCorePlugin.PLUGIN_ID, IStatus.OK,"Could not show TestResultView",null);
		}else {
