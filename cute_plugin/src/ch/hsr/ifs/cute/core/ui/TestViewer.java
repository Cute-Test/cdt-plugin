...
import ch.hsr.ifs.cute.core.model.TestStatus;
	...
	public TestViewer(Composite parent, int style, TestRunnerViewPart viewPart) {
		...
		addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				CuteCorePlugin.getModel().removeListener(TestViewer.this);
			}
		});
	}
...
//		setSize(new Point(300, 200));
	...
	public void sessionStarted(TestSession session) {
		...
		UIJob job = new UIJob("Reset TestViewer") {
			...
			public boolean belongsTo(Object family) {
				return CuteCorePlugin.PLUGIN_ID.equals(family);
			}
	...
	private TestSession getSession() {
		if(session == null) {
			session = CuteCorePlugin.getModel().getSession();
		}
		return session;
	}
