...
class WithoutCuteFileCreator extends MockFunctionFileCreator {
  ...
  public WithoutCuteFileCreator(ModificationCollector collector, CRefactoringContext cRefContext,
  ...
  private void insertTestFunDecl(IASTName functionToMock, IASTTranslationUnit newTu,
      ASTRewrite rewriter) {
  ...
  protected IASTExpressionStatement createAssertEqualStmt(String fqCallsVectorName) {
    ...
    IASTFunctionCallExpression assertEqual =
        nodeFactory.newFunctionCallExpression(createCAssert(), params);
  ...
  private static IASTInitializerClause[] createAssertEqualParams(String fqCallsVectorName) {
    ICPPASTBinaryExpression binOp =
        nodeFactory.newBinaryExpression(IASTBinaryExpression.op_equals, createExpectation(),
            createActual(fqCallsVectorName));
  ...
  private static IASTIdExpression createCAssert() {
    return nodeFactory.newIdExpression(nodeFactory.newName(MockatorConstants.C_ASSERT_EQUAL
        .toCharArray()));
  }
}
