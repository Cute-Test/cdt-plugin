...
public class ConvertToMockObjectDelegate extends MockatorDelegate {
  ...
  private void performRefactoring() {
    ...
    new MockatorRefactoringRunner(refactoring)
        .runInNewJob((edit) -> startLinkedMode(edit, refactoring.getNewMockObject().getPublicMemFuns(), refactoring.getNewMockObject()));
  }
  ...
  private void startLinkedMode(final ChangeEdit edit, final Collection<ExistingTestDoubleMemFun> memFuns, final MockObject mockObject) {
    final MockObjectLinkedEditModeFactory factory = new MockObjectLinkedEditModeFactory(edit, memFuns, getCppStd(), getAssertionOrder(),
        Optional.of(mockObject.getNameForExpectationVector()));
