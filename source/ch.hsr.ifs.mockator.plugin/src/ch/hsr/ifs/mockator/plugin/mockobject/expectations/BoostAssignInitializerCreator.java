...
public class BoostAssignInitializerCreator {
  ...
  public BoostAssignInitializerCreator(Collection<? extends TestDoubleMemFun> memFuns,
      String expectationsName, LinkedEditModeStrategy linkedEditStrategy) {
  ...
  public IASTExpressionStatement createBoostAssignInitializer() {
    ...
    IASTIdExpression vector =
        nodeFactory.newIdExpression(nodeFactory.newName(expectationsName.toCharArray()));
    ICPPASTBinaryExpression expression =
        nodeFactory.newBinaryExpression(IASTBinaryExpression.op_plusAssign, vector,
            createNextCall(head(memFuns).get(), linkedEditStrategy));
  ...
  private void addAllSignatures(ICPPASTExpressionList expressions,
      Collection<? extends TestDoubleMemFun> signatures) {
    for (TestDoubleMemFun s : signatures) {
      expressions.addExpression(createNextCall(s, linkedEditStrategy));
    }
  }

  private static ICPPASTFunctionCallExpression createNextCall(TestDoubleMemFun memFun,
      LinkedEditModeStrategy edit) {
    IASTIdExpression ctorCall =
        nodeFactory.newIdExpression(nodeFactory.newName(CALL.toCharArray()));
    IASTInitializerClause[] funArgs =
        getCallExpectations(memFun, edit).toArray(new IASTInitializerClause[] {});
    return nodeFactory.newFunctionCallExpression(ctorCall, funArgs);
  }

  private static List<IASTInitializerClause> getCallExpectations(TestDoubleMemFun memFun,
      LinkedEditModeStrategy edit) {
