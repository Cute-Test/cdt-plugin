...
public class ItaniumMangledNameGenerator {
  ...
  private void substitution(String typeStr, IType type) {
    ...
    if (typeStr.startsWith("std::basic_string<char") || typeStr.startsWith("std::allocator<char>")
        || typeStr.startsWith("std::char_traits<char>")) {
      mangledName.append("s");
  ...
  private boolean isSubstitutionNecessary(String typeStr) {
    if (typeStr.startsWith("std::basic_string") || typeStr.startsWith("std::basic_ostream<char")
        || typeStr.startsWith("std::basic_istream<char")
  ...
  private void templateArgs(Collection<ICPPTemplateArgument> templateArgs) {
    Assert.isTrue(templateArgs.size() >= 1,
        "templateArgs should not be called with empty template arg list");
    mangledName.append("I");
