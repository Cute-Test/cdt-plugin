...
public class TogglePreloadFunctionAction implements IObjectActionDelegate, IMenuCreator {
  ...
  private Collection<IProject> getReferencedShLibProjectsWithDlSupport() {
    try {
      return filter(project.getReferencedProjects(),
          (project) -> (CdtManagedProjectType.fromProject(project) != CdtManagedProjectType.SharedLib) ? false : usesDynamicLibrary(project));
