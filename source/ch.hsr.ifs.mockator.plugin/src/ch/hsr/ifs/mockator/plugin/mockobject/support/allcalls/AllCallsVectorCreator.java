...
public class AllCallsVectorCreator {
  ...
  public AllCallsVectorCreator(String nameOfAllCallsVector, IASTNode callsVectorParent,
      CppStandard cppStd) {
    this(nameOfAllCallsVector, CallsVectorParent.fromAstNode(callsVectorParent), cppStd);
  }

  public AllCallsVectorCreator(String nameOfAllCallsVector, CallsVectorParent callsVectorParent,
      CppStandard cppStd) {
  ...
  private IASTSimpleDeclaration createDeclarator(ICPPASTTemplateId stdVectorTemplateId) {
    ICPPASTNamedTypeSpecifier declSpecifier =
        nodeFactory.newTypedefNameSpecifier(stdVectorTemplateId);
    ...
    ICPPASTDeclarator declarator =
        new CPPASTDeclarator(typeDefName, cppStd.getInitializer(literal1));
  ...
  private static ICPPASTTemplateId createStdVectorOfCalls() {
    ...
    ICPPASTNamedTypeSpecifier calls =
        nodeFactory.newTypedefNameSpecifier(nodeFactory.newName(CALLS.toCharArray()));
    ICPPASTTemplateId stdVectorOfCalls =
        nodeFactory.newTemplateId(nodeFactory.newName(allCalls.toCharArray()));
