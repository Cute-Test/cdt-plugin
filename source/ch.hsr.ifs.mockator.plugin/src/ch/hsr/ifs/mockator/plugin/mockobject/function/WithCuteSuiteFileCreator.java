...
class WithCuteSuiteFileCreator extends MockFunctionFileCreator {
   ...
   public WithCuteSuiteFileCreator(ModificationCollector collector, CRefactoringContext refactoringContext, ITranslationUnit originTu,
   ...
   private static void insertCuteSuiteFunDecl(IASTTranslationUnit newTu, ASTRewrite rewriter, String suiteName) {
   ...
   private static void insertCuteSuiteFunction(IASTTranslationUnit newTu, ASTRewrite rewriter, ICPPASTFunctionDeclarator freeFunction,
         String suiteName) {
      IASTName cuteSuiteFunName = nodeFactory.newName((CUTE_SUITE_FUN_PREFIX + suiteName).toCharArray());
      ...
      ICPPASTFunctionDefinition cuiteSuiteFun = nodeFactory.newFunctionDefinition(funDeclSpec, funDecl, funBody);
   ...
   private static IASTExpressionStatement createTestFunctionRegistration(ICPPASTFunctionDeclarator funToMock, IASTIdExpression newIdExpression) {
   ...
   private static ICPPASTFunctionCallExpression createCuteTestWith(ICPPASTFunctionDeclarator funToMock) {
      IASTName cuteMacro = nodeFactory.newName(CUTE_MACRO.toCharArray());
      String testFunctionName = MockatorConstants.TEST_FUNCTION_PREFIX + funToMock.getName().toString();
   ...
   protected IASTExpressionStatement createAssertEqualStmt(String fqCallsVectorName) {
      IASTFunctionCallExpression assertEqual = nodeFactory.newFunctionCallExpression(getAssertMacro(), new IASTInitializerClause[] { createExpected(),
                                                                                                                                     createActual(
                                                                                                                                           fqCallsVectorName) });
