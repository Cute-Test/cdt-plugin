...
class MemFunBodyStrategy extends CommonFunBodyStrategy {
   ...
   protected IASTDeclarationStatement createFunTypedef(ICPPASTFunctionDeclarator funDecl) {
      ...
      String typeDefName = String.format("(%s::*%s)", String.valueOf(fullyQualifiedName.toCharArray()), FUN_PTR);
   ...
   protected IASTStatement createReinterpretCast() {
      ...
      ICPPASTUnaryExpression tmpPtr = nodeFactory.newUnaryExpression(IASTUnaryExpression.op_bracketedPrimary, createPassByPtrFor(TMP_PTR));
      args[2] = nodeFactory.newUnaryExpression(IASTUnaryExpression.op_sizeof, tmpPtr);
      IASTIdExpression memCpy = nodeFactory.newIdExpression(nodeFactory.newName("memcpy".toCharArray()));
      ICPPASTFunctionCallExpression memcpyFunCall = nodeFactory.newFunctionCallExpression(memCpy, args);
