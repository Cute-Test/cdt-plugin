...
class MissingCtorFinderVisitor extends MissingMemFunVisitor {
  ...
  public MissingCtorFinderVisitor(ICPPASTCompositeTypeSpecifier testDouble,
  ...
  private boolean hasDefaultInitTemplateParamMember() {
    ICPPASTCompositeTypeSpecifier sutClass =
        AstUtil.getChildOfType(sut, ICPPASTCompositeTypeSpecifier.class);
  ...
  private boolean isTypeNotInitialized(IASTName name) {
    ...
    return !(initializer instanceof ICPPASTConstructorInitializer)
        || hasEmptyInitializer(initializer);
  }
  ...
  private int handleSimpleDecl(ICPPASTConstructorInitializer ctorInitializer) {
    ICPPASTDeclarator declarator =
        AstUtil.getAncestorOfType(ctorInitializer, ICPPASTDeclarator.class);
  ...
  private int handleCtorInitializer(IASTInitializer initializer,
      ICPPASTConstructorInitializer ctorInitializer) {
    ICPPASTConstructorChainInitializer ctor =
        AstUtil.getAncestorOfType(ctorInitializer, ICPPASTConstructorChainInitializer.class);
  ...
  public int visit(IASTDeclaration decl) {
    if (!(decl instanceof IASTSimpleDeclaration && AstUtil.isPartOf(decl,
        ICPPASTFunctionDefinition.class)))
      return PROCESS_CONTINUE;

    ICPPASTConstructorInitializer ctorInit =
        AstUtil.getChildOfType(decl, ICPPASTConstructorInitializer.class);
    ICPPASTFunctionCallExpression funCall =
        AstUtil.getChildOfType(decl, ICPPASTFunctionCallExpression.class);
  ...
  private boolean hasTemplateParamType(IASTName name) {
    return getType(name) instanceof ICPPTemplateParameter
        && name.toString().equals(getTemplateParamName());
  }
  ...
  private Constructor createCtorWith(IASTInitializer initializer) {
    ...
    ICPPASTFunctionCallExpression call =
        nodeFactory.newFunctionCallExpression(idExpr,
            arguments.toArray(new IASTInitializerClause[arguments.size()]));
  ...
  private static IASTNode getParent(IASTNode ctor) {
    ICPPASTFunctionDefinition parentFunction =
        AstUtil.getAncestorOfType(ctor, ICPPASTFunctionDefinition.class);
