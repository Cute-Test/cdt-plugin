...
class MissingOperatorFinderVisitor extends MissingMemFunVisitor {
  ...
  public MissingOperatorFinderVisitor(ICPPASTCompositeTypeSpecifier testDouble,
  ...
  private int handleIndexOperator(ICPPASTArraySubscriptExpression expr) {
    if (resolvesToTemplateType(expr.getArrayExpression())
        && expr.getExpressionType() instanceof TypeOfDependentExpression) {
  ...
  private int handleFunCallOperator(ICPPASTFunctionCallExpression funCall) {
    ...
    if (resolvesToTemplateType(functionNameExpression)
        && expressionType instanceof TypeOfDependentExpression) {
  ...
  private int handleBinaryOperator(ICPPASTBinaryExpression binEx) {
    ...
    if (isAppropriateType(binEx.getOperand1())) {
      OverloadableOperator operator =
          OverloadableOperator.fromBinaryExpression(binEx.getOperator());
  ...
  private static boolean shouldSkipUnaryOperator(ICPPASTUnaryExpression uExpr,
      OverloadableOperator operator) {
