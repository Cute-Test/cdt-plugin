...
public class MacroOptionHandler extends AbstractOptionsHandler {
  ...
  private void toggleMacroWrapFunName(final F2<String, Collection<String>, Void> macroOp,
      final String macroName) {
    withEveryTool(new F2<ITool, IConfiguration, Void>() {
      @Override
      public Void apply(ITool tool, IConfiguration config) {
        IOption flagsOption =
            tool.getOptionBySuperClassId(projectVariables.getPreprocessorDefinesId());
