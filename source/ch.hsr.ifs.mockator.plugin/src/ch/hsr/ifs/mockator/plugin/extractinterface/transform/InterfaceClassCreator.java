...
public class InterfaceClassCreator implements F1V<ExtractInterfaceContext> {
  ...
  private static void addPureVirtualMemFuns(Collection<IASTDeclaration> publicMemFuns,
      ICPPASTCompositeTypeSpecifier newInterface, ICPPASTCompositeTypeSpecifier klass) {
    for (IASTDeclaration memFun : publicMemFuns) {
      ICPPASTFunctionDeclarator oldDeclarator =
          AstUtil.getChildOfType(memFun, ICPPASTFunctionDeclarator.class);
      ICPPASTFunctionDeclarator newDeclarator =
          createNewFunDeclarator(oldDeclarator, newInterface.getName().toString(), klass);
      ICPPASTDeclSpecifier newDeclSpec =
          createNewFunDeclSpecifier(newInterface, klass, oldDeclarator, memFun);
  ...
  private static ICPPASTFunctionDeclarator createNewFunDeclarator(
      ICPPASTFunctionDeclarator oldDecl, String newInterfaceName,
  ...
  private static void copyPointers(ICPPASTFunctionDeclarator oldDecl,
      ICPPASTFunctionDeclarator newDecl) {
    for (IASTPointerOperator pOp : oldDecl.getPointerOperators()) {
      newDecl.addPointerOperator(pOp.copy());
    }
  }

  private static void copyExceptionSpecifications(ICPPASTFunctionDeclarator declarator,
      ICPPASTFunctionDeclarator newDecl) {
  ...
  private static void adaptParametersIfNecessary(String newInterfaceName,
      ICPPASTFunctionDeclarator oldDecl, ICPPASTFunctionDeclarator newDecl,
      ICPPASTCompositeTypeSpecifier klass) {
  ...
  private static Collection<ICPPASTParameterDeclaration> createNewParameters(
      ICPPASTFunctionDeclarator funDecl, String newInterfaceName,
      ICPPASTCompositeTypeSpecifier klass) {
  ...
  private static boolean hasPointerOrReferenceToInterface(ICPPASTDeclarator declarator,
      IASTDeclSpecifier declSpecifier, ICPPASTCompositeTypeSpecifier klass) {
  ...
  private static ICPPASTDeclSpecifier createNewFunDeclSpecifier(
      ICPPASTCompositeTypeSpecifier newInterface, ICPPASTCompositeTypeSpecifier klass,
      ICPPASTFunctionDeclarator declarator, IASTDeclaration declaration) {
    ICPPASTDeclSpecifier oldDeclSpec =
        AstUtil.getChildOfType(declaration, ICPPASTDeclSpecifier.class);
  ...
  private static IASTNode createInterfaceDeclWithNamespaces(ICPPASTCompositeTypeSpecifier newClass,
      ICPPASTCompositeTypeSpecifier klass) {
  ...
  private static void insertWithIncludeGuards(IASTNode interfaceClass,
    ...
    IncludeGuardCreator creator =
        new IncludeGuardCreator(getInterfaceFile(context), context.getCProject());
  ...
  private static void insertIncludeGuardEnd(IncludeGuardCreator creator, ASTRewrite rewriter,
      IASTTranslationUnit tu) {
    rewriter.insertBefore(tu, null, creator.createEndIf(), null);
  }

  private static void insertInterface(IASTNode interfaceClass, ASTRewrite rewriter,
      IASTTranslationUnit tuOfInterface) {
    rewriter.insertBefore(tuOfInterface, null, interfaceClass, null);
  }

  private static IASTTranslationUnit insertIncludeGuardStart(IncludeGuardCreator creator,
      ASTRewrite rewriter, IASTTranslationUnit tu) {
    rewriter.insertBefore(tu, null, creator.createIfNDef(), null);
    rewriter.insertBefore(tu, null, creator.createDefine(), null);
    return tu;
  }

  private static void insertDeclarations(Collection<IASTSimpleDeclaration> declarations,
      ASTRewrite rewriter, IASTTranslationUnit tuOfInterface) {
    for (IASTSimpleDeclaration decl : declarations) {
      rewriter.insertBefore(tuOfInterface, null, decl.copy(), null);
    }
  }

  private static void insertIncludes(Collection<IASTPreprocessorIncludeStatement> includes,
      ASTRewrite rewriter, IASTTranslationUnit tuOfInterface) {
