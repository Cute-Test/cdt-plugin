...
class InitializerExpectationsReconciler extends AbstractExpectationsReconciler {
  ...
  public InitializerExpectationsReconciler(ASTRewrite rewriter,
  ...
  private static IASTEqualsInitializer getEqualsInitializer(IASTName expVector) {
    IASTDeclarationStatement declStmt =
        AstUtil.getAncestorOfType(expVector, IASTDeclarationStatement.class);
    IASTEqualsInitializer eqInitializer =
        AstUtil.getChildOfType(declStmt, IASTEqualsInitializer.class);
  ...
  private static ICPPASTInitializerList getInitializerListFrom(IASTEqualsInitializer eqInitializer) {
    Assert.instanceOf(eqInitializer.getInitializerClause(), ICPPASTInitializerList.class,
  ...
  private void collectNewCalls(ICPPASTInitializerList newCallsList) {
    for (TestDoubleMemFun toAdd : callsToAdd) {
      ...
      call.addClause(nodeFactory.newLiteralExpression(IASTLiteralExpression.lk_string_literal,
          StringUtil.quote(toAdd.getFunctionSignature())));
  ...
  private void collectRegisteredCalls(ICPPASTInitializerList callsInitializerList,
      ICPPASTInitializerList newCallsList) {
  ...
  private void replaceCallsInitializer(IASTEqualsInitializer eqInitializer,
      ICPPASTInitializerList newCallsList) {
