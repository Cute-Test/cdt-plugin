...
public class RemoveInitMockatorRefactoring extends MockatorRefactoring {
   ...
   public RemoveInitMockatorRefactoring(IDocument doc, ICElement cElement, ITextSelection selection, ICProject cProject) {
   ...
   private ICPPASTFunctionDefinition getUpdatedTestFunction(ICPPASTFunctionDefinition testFunction) {
      try {
         ...
         Collection<IASTFunctionDefinition> testFunctions = filter(getTestfunctionsInTu(), new F1<IASTFunctionDefinition, Boolean>() {
   ...
   private static DeleteEdit createDeleteEdit(int funNodeOffset, String funText, String initMockatorMacro) {
   ...
   private IASTFunctionDefinition getMatchingTestFunction(Collection<IASTFunctionDefinition> allTestFunctions) {
      final String testFunName = testFunction.getDeclarator().getName().toString();
      Collection<IASTFunctionDefinition> functions = filter(allTestFunctions, new F1<IASTFunctionDefinition, Boolean>() {
   ...
   private Collection<IASTFunctionDefinition> getAllTestFunctions(IProgressMonitor pm) throws CoreException {
      TestFunctionFinderVisitor finder = new TestFunctionFinderVisitor(getFunctionsToAnalyze());
   ...
   private TextFileChange createTextFileChange(MultiTextEdit multiTextEdit, IProgressMonitor pm) throws CoreException {
      IASTTranslationUnit ast = getAST(tu, pm);
      TextFileChange change = new TextFileChange("Delete mockator init call", FileUtil.toIFile(ast.getFilePath()));
