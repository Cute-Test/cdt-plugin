...
public class RemoveInitMockatorRefactoring extends MockatorRefactoring {
  ...
  private ICPPASTFunctionDefinition getUpdatedTestFunction(final ICPPASTFunctionDefinition testFunction) {
    try {
      ...
      final Collection<IASTFunctionDefinition> testFunctions = filter(getTestfunctionsInTu(),
          (funDef) -> verifier.isEquivalent((ICPPASTFunctionDeclarator) funDef.getDeclarator()));
  ...
  private IASTFunctionDefinition getMatchingTestFunction(final Collection<IASTFunctionDefinition> allTestFunctions) {
    ...
    final Collection<IASTFunctionDefinition> functions = filter(allTestFunctions,
        (funDef) -> funDef.getDeclarator().getName().toString().equals(testFunName));
