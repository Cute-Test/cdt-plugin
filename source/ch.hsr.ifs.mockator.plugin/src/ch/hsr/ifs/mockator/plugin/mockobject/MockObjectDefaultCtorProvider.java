...
public class MockObjectDefaultCtorProvider implements DefaultCtorProvider {
   ...
   private boolean hasPublicCtors(final Collection<? extends MissingMemberFunction> memFuns) {
      final Collection<ICPPASTFunctionDefinition> onlyCtors = filter(toFunctions(memFuns), new F1<ICPPASTFunctionDefinition, Boolean>() {

         @Override
         public Boolean apply(final ICPPASTFunctionDefinition function) {
   ...
   private <T extends MissingMemberFunction> Collection<ICPPASTFunctionDefinition> toFunctions(final Collection<T> memFuns) {
      return map(memFuns, new F1<T, ICPPASTFunctionDefinition>() {
         ...
         @Override
         public ICPPASTFunctionDefinition apply(final T missingMemFun) {
