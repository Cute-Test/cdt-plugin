...
public class FunctionEquivalenceVerifier {
  ...
  public boolean isEquivalent(ICPPASTFunctionDeclarator other) {
    if (!funDecl.getName().toString().equals(other.getName().toString()))
      return false;

    ParamTypeEquivalenceTester tester =
        new ParamTypeEquivalenceTester(getParameterTypes(funDecl), getParameterTypes(other));
  ...
  public boolean isEquivalent(ICPPASTFunctionCallExpression functionCall,
      ConstStrategy constStrategy) {
    if (!funDecl.getName().toString().equals(AstUtil.getName(functionCall).toString()))
      return false;
  ...
  private static boolean areParamsEquivalentToArguments(ICPPASTFunctionCallExpression funCall,
