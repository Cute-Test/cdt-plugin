...
class FreeFunBodyStrategy extends CommonFunBodyStrategy {
   ...
   protected IASTExpressionStatement createReinterpretCast() {
      ICPPASTTypeId funPtr = nodeFactory.newTypeId(nodeFactory.newSimpleDeclSpecifier(), nodeFactory.newDeclarator(nodeFactory.newName(FUN_PTR
            .toCharArray())));
      IASTIdExpression tmpPtr = nodeFactory.newIdExpression(nodeFactory.newName(TMP_PTR.toCharArray()));
      ICPPASTCastExpression reinterpretCast = nodeFactory.newCastExpression(ICPPASTCastExpression.op_reinterpret_cast, funPtr, tmpPtr);
