...
class StaticPolymorphismUseFinder implements
    F1<IASTFunctionDefinition, Collection<StaticPolyMissingMemFun>> {
  private final ICProject cProject;
  private final ICPPASTCompositeTypeSpecifier testDouble;
  private final IIndex index;

  public StaticPolymorphismUseFinder(ICPPASTCompositeTypeSpecifier testDouble, ICProject cProject,
      IIndex index) {
  ...
  private Collection<StaticPolyMissingMemFun> collectUsedMemFunsInSut(
      Pair<ICPPASTTemplateDeclaration, ICPPASTTemplateParameter> declParam) {
  ...
  private static Collection<ICPPASTTemplateDeclaration> getTemplateFunctions(
  ...
  private Collection<Pair<ICPPASTTemplateDeclaration, ICPPASTTemplateParameter>> getTestDoubleAsTemplateArgUsages(
    ...
    ClassInTemplateIdFinderVisitor finder =
        new ClassInTemplateIdFinderVisitor(testDouble, cProject, index);
    testFunction.accept(finder);
    return finder.getTemplateParamCombinations();
  }

  private Collection<StaticPolyMissingMemFun> filterNotReferenced(
      Collection<StaticPolyMissingMemFun> missingMemFuns, IASTTranslationUnit tuOfTemplate,
      IASTFunctionDefinition testFunction) {
  ...
  private NotReferencedFunctionFilter getNotReferencedFunFilter(IASTTranslationUnit tuOfTemplate,
      IASTFunctionDefinition testFunction) {
    return new NotReferencedFunctionFilter(index, cProject,
