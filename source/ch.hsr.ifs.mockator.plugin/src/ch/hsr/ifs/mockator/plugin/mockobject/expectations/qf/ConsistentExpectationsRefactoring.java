...
class ConsistentExpectationsRefactoring extends MockatorRefactoring {
  ...
  public ConsistentExpectationsRefactoring(final ICElement cElement, final ITextSelection selection, final ICProject project,
      final ConsistentExpectationsCodanArgs ca, final CppStandard cppStd, final LinkedEditModeStrategy linkedEditMode) {
  ...
  @Override
  protected void collectModifications(final IProgressMonitor pm, final ModificationCollector collector)
  ...
  private Collection<ExistingMemFunCallRegistration> collectRegisteredCalls(final IASTName expectationsVector, final IASTTranslationUnit ast) {
    ...
    return OptHelper.returnIfPresentElse(getRegistrationVector(testFunction, expectationsVector),
        (regVector) -> finder.findCallRegistrations(regVector), () -> list());
  }
