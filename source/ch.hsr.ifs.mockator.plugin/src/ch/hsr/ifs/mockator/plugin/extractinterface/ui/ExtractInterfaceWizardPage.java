...
class ExtractInterfaceWizardPage extends UserInputWizardPage {
  ...
  private void createInterfaceNameField(Composite parent) {
    ...
    interfaceNameField =
        new LabeledTextField(interfaceNameContent, I18N.ExtractInterfaceName,
            getNewInterfaceClassProposal());
  ...
  private void createReplaceAllCheckbox(Composite parent) {
    ...
    replaceAllCheckbox.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent e) {
        getMyRefactoring().getContext().setShouldReplaceAllOccurences(
            replaceAllCheckbox.getSelection());
      }
    });
  }
  ...
  private void createMemFunTable(Composite parent) {
    memFunsTableViewer =
        CheckboxTableViewer.newCheckList(parent, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL);
  ...
  private void selectUsedMemFuns() {
    Collection<IASTDeclaration> usedMemFuns =
        getMyRefactoring().getContext().getUsedPublicMemFuns();
  ...
  private void toogleSelectionButtons() {
    ...
    selectAllButton.setEnabled(availableMemFuns != null
        && checkedMemFuns.size() < availableMemFuns.size());
    ...
    return new LabelProvider() {
      ...
      public String getText(Object element) {
        ICPPASTFunctionDeclarator funDecl =
            AstUtil.getChildOfType((IASTNode) element, ICPPASTFunctionDeclarator.class);
  ...
  private void createSelectButtons(Composite parent) {
    ...
    deselectAllButton =
        createSelectionButton(buttonComposite, I18N.ExtractInterfaceDeselectAll, false);
  }
