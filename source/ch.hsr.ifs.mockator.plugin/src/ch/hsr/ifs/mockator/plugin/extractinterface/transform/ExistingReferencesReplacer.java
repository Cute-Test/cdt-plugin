...
public class ExistingReferencesReplacer implements F1V<ExtractInterfaceContext> {
   ...
   private static void addLocalUsagesIfNecessary(ExtractInterfaceContext context, IASTName className, Collection<IASTName> usages) {
   ...
   private static void handleSimpleDecl(String newInterfaceName, ASTRewrite rewriter, IASTSimpleDeclaration simpleDecl, IASTName astName) {
   ...
   private static void handleClassFwdDecl(String newInterfaceName, ASTRewrite rewriter, ICPPASTElaboratedTypeSpecifier declSpecifier) {
   ...
   private static void handleReturnType(String interfaceName, ASTRewrite rewriter, ICPPASTFunctionDefinition funDef) {
      ...
      ICPPASTNamedTypeSpecifier newSpecifier = createNewNamedType(interfaceName, (ICPPASTNamedTypeSpecifier) declSpec);
      rewriter.replace(declSpec, newSpecifier, null);
   }

   private static void handleParameter(String newInterfaceName, ASTRewrite rewriter, ICPPASTParameterDeclaration paramDecl, IASTName astName) {
      if (hasTemplateId(paramDecl)) {
         handleTemplateId(newInterfaceName, rewriter, paramDecl, astName);
      } else {
         ICPPASTNamedTypeSpecifier newSpecifier = createNewNamedType(newInterfaceName, (ICPPASTNamedTypeSpecifier) paramDecl.getDeclSpecifier());
   ...
   private static ICPPASTNamedTypeSpecifier createNewNamedType(String newInterfaceName, ICPPASTDeclSpecifier declSpec) {
   ...
   private static ICPPASTQualifiedName getNewQNameForInterface(ICPPASTQualifiedName qName, String newInterfaceName) {
      ICPPASTQualifiedName newQfName = nodeFactory.newQualifiedName(nodeFactory.newName(newInterfaceName.toCharArray()));
