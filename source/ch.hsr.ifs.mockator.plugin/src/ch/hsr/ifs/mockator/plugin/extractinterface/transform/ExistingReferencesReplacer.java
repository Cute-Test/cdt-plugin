...
public class ExistingReferencesReplacer implements F1V<ExtractInterfaceContext> {
  ...
  private static Collection<IASTName> getUsagesOfConcreteType(ExtractInterfaceContext context) {
    ...
    Collection<IASTName> usages =
        lookup.findReferencingNames(chosenClassName, context.getCRefContext());
  ...
  private static void addLocalUsagesIfNecessary(ExtractInterfaceContext context,
      IASTName className, Collection<IASTName> usages) {
    if (!usages.isEmpty())
      return;
  ...
  private static boolean isPointerOrRefOrFwdDeclToChosenClass(IASTName usage,
  ...
  private static void replaceDeclarationWithNewType(IASTName name, ExtractInterfaceContext context) {
    ...
    if (declaration instanceof IASTSimpleDeclaration) {
      handleSimpleDecl(context.getNewInterfaceName(), rewriter,
          (IASTSimpleDeclaration) declaration, name);
    } else if (declaration instanceof ICPPASTParameterDeclaration) {
      handleParameter(context.getNewInterfaceName(), rewriter,
          (ICPPASTParameterDeclaration) declaration, name);
    } else if (declaration instanceof ICPPASTFunctionDefinition) {
      handleReturnType(context.getNewInterfaceName(), rewriter,
          (ICPPASTFunctionDefinition) declaration);
    }
  }

  private static ICPPASTNamedTypeSpecifier getTypeSpecIfRefersToClass(IASTNode selectedNode,
      IASTName className) {
  ...
  private static void handleSimpleDecl(String newInterfaceName, ASTRewrite rewriter,
      IASTSimpleDeclaration simpleDecl, IASTName astName) {
  ...
  private static void handleReturnType(String interfaceName, ASTRewrite rewriter,
      ICPPASTFunctionDefinition funDef) {
    IASTDeclSpecifier declSpec = funDef.getDeclSpecifier();
    ICPPASTNamedTypeSpecifier newSpecifier =
        createNewNamedType(interfaceName, (ICPPASTNamedTypeSpecifier) declSpec);
  ...
  private static void handleParameter(String newInterfaceName, ASTRewrite rewriter,
    ...
    if (hasTemplateId(paramDecl)) {
      ...
      ICPPASTNamedTypeSpecifier newSpecifier =
          createNewNamedType(newInterfaceName,
              (ICPPASTNamedTypeSpecifier) paramDecl.getDeclSpecifier());
      rewriter.replace(paramDecl.getDeclSpecifier(), newSpecifier, null);
    }
  }

  private static void handleTemplateId(String newName, ASTRewrite rewriter, IASTNode parentDecl,
      IASTName astName) {
  ...
  private static ICPPASTTypeId getType(String newInterfaceName, IASTName astName,
      IASTNode templateArg) {
  ...
  private static ICPPASTQualifiedName getNewQNameForInterface(ICPPASTQualifiedName qName,
      String newInterfaceName) {
    ICPPASTQualifiedName newQfName = nodeFactory.newQualifiedName();
    List<IASTName> qNameWithoutClassName =
        list(qName.getNames()).subList(0, qName.getNames().length - 1);

    for (IASTName name : qNameWithoutClassName) {
      newQfName.addName(name);
    }

    newQfName.addName(nodeFactory.newName(newInterfaceName.toCharArray()));
    return newQfName;
  }
}
