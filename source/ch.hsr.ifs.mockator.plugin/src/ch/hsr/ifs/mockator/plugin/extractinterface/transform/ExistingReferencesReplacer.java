...
public class ExistingReferencesReplacer implements F1V<ExtractInterfaceContext> {
  ...
  private static void replaceDeclarationWithNewType(IASTName name,
      ExtractInterfaceContext context) {
    ASTRewrite rewriter = context.getRewriterFor(name.getTranslationUnit());
    IASTNode declaration = getDeclaration(name);

    if (declaration instanceof IASTSimpleDeclaration) {
      handleSimpleDecl(context.getNewInterfaceName(), rewriter, (IASTSimpleDeclaration) declaration,
          name);
  ...
  private static ICPPASTQualifiedName getNewQNameForInterface(ICPPASTQualifiedName qName,
    ...
    ICPPASTQualifiedName newQfName =
        nodeFactory.newQualifiedName(nodeFactory.newName(newInterfaceName.toCharArray()));
