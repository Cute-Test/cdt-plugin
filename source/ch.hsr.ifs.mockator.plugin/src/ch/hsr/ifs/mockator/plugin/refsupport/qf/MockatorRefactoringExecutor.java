...
class MockatorRefactoringExecutor implements F2<MockatorRefactoring, IProgressMonitor, ChangeEdit> {
  ...
  private static void assureChangeObjectIsValid(Change change, IProgressMonitor pm)
      throws CoreException {
    if (!change.isValid(pm).isOK())
      throw new MockatorException("Change object is invalid");
  }

  private static void prepareUndo(String refactoringDesc, Change change, Change undoChange,
      boolean success, IProgressMonitor pm) {
  ...
  private static void assurePreconditions(MockatorRefactoring refactoring,
  ...
  private static Change createChangeObject(MockatorRefactoring refactoring,
      CRefactoringContext context, IProgressMonitor pm) {
    try {
      return refactoring.createChange(pm);
    } catch (CoreException e) {
      throw new MockatorException("Creating change object failed: " + refactoring.getDescription(),
          e);
    } finally {
      context.dispose();
    }
