...
class Operator extends AbstractStaticPolyMissingMemFun {
  ...
  public Operator(IASTExpression operatorExpr, OverloadableOperator operator, IType injectedType,
  ...
  private ICPPASTDeclSpecifier lookupReturnType(ICPPASTFunctionDeclarator funDecl) {
    return new ReturnTypeDeducer(funDecl, injectedType, memberClassName)
        .determineReturnType(operatorExpr);
  }

  @Override
  protected IASTCompoundStatement createFunBody(TestDoubleMemFunImplStrategy strategy,
      ICPPASTFunctionDeclarator funDecl, ICPPASTDeclSpecifier returnType, CppStandard cppStd) {
  ...
  private void addReturnStmtToFunBody(CppStandard cppStd, ICPPASTDeclSpecifier returnType,
      IASTCompoundStatement funBody, ICPPASTFunctionDeclarator funDecl) {
  ...
  private void addParameterForArrayIndex(ICPPASTFunctionDeclarator funDecl) {
    ...
    funDecl.addParameterDeclaration(ParamDeclCreator.createParameterFrom(expr,
  ...
  private void addParamsForFunArgs(ICPPASTFunctionDeclarator funDecl) {
    FunctionCallParameterCollector ex =
        new FunctionCallParameterCollector((ICPPASTFunctionCallExpression) operatorExpr);
  ...
  private static void handleLiteralParam(ICPPASTFunctionDeclarator funDecl,
      IASTExpression binOp2Expr, Map<String, Boolean> nameHistory) {
    IASTLiteralExpression litexpr = (IASTLiteralExpression) binOp2Expr;
    ICPPASTParameterDeclaration literalParam =
        ParamDeclCreator.createParameter(litexpr, nameHistory);
    funDecl.addParameterDeclaration(literalParam);
  }

  private void handleNamedParam(ICPPASTFunctionDeclarator funDecl, IASTExpression binOp2Expr,
      Map<String, Boolean> nameHistory) {
  ...
  private void addReferenceParamToThisClass(ICPPASTFunctionDeclarator funDecl,
      Map<String, Boolean> nameHistory) {
    ICPPASTParameterDeclaration referenceToThis =
        ParamDeclCreator.createReferenceParamFrom(memberClassName, nameHistory);
  ...
  private static void addEmptyIntParameter(ICPPASTFunctionDeclarator decl) {
    ...
    ICPPASTParameterDeclaration emptyIntParam =
        nodeFactory.newParameterDeclaration(getDefaultType(), emptyDecl);
  ...
  private boolean isPostfixOperator() {
    ...
    return uExpr != null
        && (uExpr.getOperator() == IASTUnaryExpression.op_postFixDecr || uExpr.getOperator() == IASTUnaryExpression.op_postFixIncr);
  }

  private boolean shouldYieldByPointer() {
    return isUnaryExpression()
        && getUnaryExpression().getOperator() == IASTUnaryExpression.op_amper;
  }
