...
public:
	xml_output_file(const std::string& cppStd) :
			fileName("test_results_" + cppStd + ".xml"), cppStd(cppStd) {
		openFile();
		writePrologue();
	}
	...
	void testsuiteStart(const std::string& name, const int size) {
		resultFile << ONE_TAB << "<testsuite name=\"" << name << "\" tests=\""
				<< size << "\">" << std::endl;
	}
	...
	void testcaseStart(const std::string& classname,
			const std::string& testname) {
		resultFile << TWO_TABS << "<testcase classname=\"" << "ch.hsr.ifs.mockator.lib." << cppStd
				<< "\" name=\"" << testname << "\"";
	}
	...
	void testcaseFailure(const std::string & filename, const int lineno,
			const std::string & reason) {
		resultFile << ">" << std::endl;
		resultFile << THREE_TABS << "<failure message=\"" << filename << ":"
				<< lineno << " " << reason << "\"/>" << std::endl;
		testcaseEnd();
	}

	void testcaseError(const std::string& testname, const std::string& what) {
		resultFile << ">" << std::endl;
		resultFile << THREE_TABS << "<failure message=\"" << testname << " "
				<< what << "\"/>" << std::endl;
		testcaseEnd();
	}
