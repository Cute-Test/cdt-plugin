...
public class CuteProgressBar extends Canvas implements ITestElementListener, ISessionListener, ITestCompositeListener {
   ...
   private void setStatusColor(GC gc) {
      if (fStopped) gc.setBackground(fStoppedColor);
      else if (fError) gc.setBackground(fFailureColor);
      else gc.setBackground(fOKColor);
   }
   ...
   private int scale(int value) {
      if (fMaxTickCount > 0) {
         ...
         if (r.width != 0) return Math.max(0, value * (r.width - 2) / fMaxTickCount);
      }
      return value;
   }
