...
public class CuteConsoleEventParser extends ConsoleEventParser {
   ...
   public final String getComprehensiveLinePattern() {
      return escapeForRegex(LINE_QUALIFIER + "(" + regExUnion(new String[] { LINE_QUALIFIER, BEGINNING, ENDING, SUCCESS, STARTTEST, FAILURE,
                                                                             ERROR, })) + ")(.*)(\\n)";
   }
   ...
   protected void extractTestEventsFor(IRegion reg, String line) throws CoreException {
      ...
      else if (suiteStarting(line)) suiteStarted(reg, line);
      else if (suiteEnding(line)) suiteEnded(reg, line);
      else if (testErrored(line)) testError(reg, line);
   }
