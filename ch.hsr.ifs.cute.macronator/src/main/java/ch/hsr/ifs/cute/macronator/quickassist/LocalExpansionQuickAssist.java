...
import ch.hsr.ifs.cute.macronator.refactoring.ExpandMacroProposal;
...
public class LocalExpansionQuickAssist implements IQuickAssistProcessor {
    ...
    public ICCompletionProposal[] getAssists(final IInvocationContext context, IProblemLocation[] locations) throws CoreException {
        IStatus status = ASTProvider.getASTProvider().runOnAST(context.getTranslationUnit(), ASTProvider.WAIT_ACTIVE_ONLY, new NullProgressMonitor(), new CompletionProposoalAstRunnable(context.getSelectionOffset(), context.getSelectionLength()));
