...
void assert_relop(TL const &left
	...
	throw test_failure(cute_to_string::backslashQuoteTabNewline(msg)
	                   + cute_relops_detail::compare_values(left,right),file,line);
}

}
#define ASSERT_LESSM(msg,left,right) cute::assert_relop<std::less>((left),(right),msg,__FILE__,__LINE__)
#define ASSERT_LESS(left,right) ASSERT_LESSM(#left " < " #right, (left),(right))
#define ASSERT_LESS_EQUALM(msg,left,right) cute::assert_relop<std::less_equal>((left),(right),msg,__FILE__,__LINE__)
#define ASSERT_LESS_EQUAL(left,right) ASSERT_LESS_EQUALM(#left " <= " #right, (left),(right))
#define ASSERT_GREATERM(msg,left,right) cute::assert_relop<std::greater>((left),(right),msg,__FILE__,__LINE__)
#define ASSERT_GREATER(left,right) ASSERT_GREATERM(#left " > " #right, (left),(right))
#define ASSERT_GREATER_EQUALM(msg,left,right) cute::assert_relop<std::greater_equal>((left),(right),msg,__FILE__,__LINE__)
#define ASSERT_GREATER_EQUAL(left,right) ASSERT_GREATER_EQUALM(#left " >= " #right, (left),(right))
#define ASSERT_NOT_EQUAL_TOM(msg,left,right) cute::assert_relop<std::not_equal_to>((left),(right),msg,__FILE__,__LINE__)
