...
public class IndexFinder {
	...
	private static void findNames(IASTName name, ASTRewriteCache rewriteCache, ResultHandler resultHandler, int flags) {
		try {
			...
			for(IIndexName indexName : indexNames) {
				...
				IASTName declarationName = (IASTName)ASTAnalyzer.getMarkedNode(astTranslationUnit, indexName.getNodeOffset(), indexName.getNodeLength());
	...
	public static IIndexBinding[] findBindings(IASTName name, boolean isPrefix) throws Exception {
		...
		if(adaptedBinding != null) {
			bindings = index.findBindings(adaptedBinding.getQualifiedNameCharArray(), IndexFilter.ALL_DECLARED, null);
		}
		else {
			bindings = new IIndexBinding[0];
		}
		return bindings;
	}
}