...
void thisIsAtest_cute_filter_runnerTest() {
	char const *argv[] = { "dummy", "testsuite1", "testsuite2#test1", 0,
...
void testIfRunnerReallyFilters(){
	char const *argv[] = { "dummy", "AllTests#test1", 0 };
	int argc=2;
	cute::suite s;
	...
	cute::runner<cute::counting_listener<> > run = cute::makeRunner(l,
			argc, argv);
	run(s,"AllTests");
	ASSERT_EQUAL(1,l.numberOfSuites);
	ASSERT_EQUAL(1,l.numberOfTests);
	ASSERT_EQUAL(1,l.failedTests);
}
...
cute::suite make_suite_test_cute_filter_runner() {
	...
	s.push_back(CUTE(testIfRunnerReallyFilters));
	return s;
}
