...
import ch.hsr.ifs.cute.templator.plugin.asttools.IIndexAction;
import ch.hsr.ifs.cute.templator.plugin.asttools.IndexAction;
import ch.hsr.ifs.cute.templator.plugin.asttools.data.ASTCache;
...
public class DefinitionFinder {
	...
	public IASTName findDefinition(IBinding binding) throws TemplatorException {
		...
		if (definitionNames.length != 1) {
			throw new TemplatorException(
					"Found " + definitionNames.length + " definitions for " + binding + ", expected 1");
		}
	...
	private static IASTName findDefinitionWithNodeSelector(IASTTranslationUnit astForExternalFile,
			IIndexName indexNameForTemplateBinding) throws TemplatorException {
		int offset = indexNameForTemplateBinding.getNodeOffset();
		int length = indexNameForTemplateBinding.getNodeLength();

		IASTNodeSelector nodeSelector = astForExternalFile
				.getNodeSelector(indexNameForTemplateBinding.getFileLocation().getFileName());
		IASTName functionDefinitionName = nodeSelector.findEnclosingName(offset, length);

		if (functionDefinitionName == null) {
			throw new TemplatorException(
					"Could not find an enclosing name with offset " + offset + " and length " + length);
		}
	...
	private IIndexName findIndexName(final IBinding binding) throws TemplatorException {
		...
		if (foundIndexNames == null || foundIndexNames.length != 1) {
			throw new TemplatorException(
					"Could not find index name for " + binding.getName() + ". Maybe try rebuilding the index.");
		}
