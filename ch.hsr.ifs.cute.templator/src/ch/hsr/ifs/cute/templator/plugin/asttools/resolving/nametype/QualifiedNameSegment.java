...
import ch.hsr.ifs.cute.templator.plugin.asttools.resolving.ClassTemplateMemberResolver;
...
public class QualifiedNameSegment extends TypeNameToType {
	...
	public QualifiedNameSegment(IASTName typeName, AbstractResolvedNameInfo currentContext,
			QualifiedNameSegment lastSegment) {
	...
	public QualifiedNameSegment resolve(NameTypeDeducer typeDeducer) throws TemplatorException {
		...
		if (unresolvedCurrentSegmentType instanceof ICPPDeferredClassInstance) {
			...
			if (memberClassInstance instanceof ICPPAliasTemplateInstance) {
				TypeNameToType aliasType = getAliasTemplateInstanceType((ICPPAliasTemplateInstance) memberClassInstance,
						typeName, currentContext, lastSegment, typeDeducer);
				memberClassInstance = aliasType.getSpecialization();
			}

			AbstractResolvedNameInfo newContext = typeDeducer.createNewContext(typeName, memberClassInstance,
					currentContext, true);
			...
			ICPPTemplateArgument argument = currentContext
					.getArgument((ICPPTemplateParameter) unresolvedCurrentSegmentType);
	...
	private TypeNameToType getAliasTemplateInstanceType(ICPPAliasTemplateInstance aliasInstance, IASTName typeName,
			AbstractResolvedNameInfo parent, QualifiedNameSegment lastSegment, NameTypeDeducer typeDeducer)
		...
		if (aliasedTypeName.getParent() instanceof ICPPASTQualifiedName) {
			...
			AbstractResolvedNameInfo aliasContext = typeDeducer.createNewContext(typeName, aliasInstance,
					contextFromLastSegment, false);
