...
public class TemplateContextLookupData extends LookupData {
	...
	public TemplateContextLookupData(IASTName name, ICPPTemplateParameterMap templateParameterMap,
			IType impliedObjectType) {
		super(name);
	...
	private IType replaceDependentType(IType functionArgumentType) {
		...
		if (adaptedType instanceof CPPQualifierType) {
			...
			return new CPPQualifierType(replaceDependentType(qualifierType.getType()), qualifierType.isConst(),
					qualifierType.isVolatile());
		}
		if (adaptedType instanceof CPPPointerType) {
			CPPPointerType pointerType = (CPPPointerType) adaptedType;
			return new CPPPointerType(replaceDependentType(pointerType.getType()), pointerType.isConst(),
					pointerType.isVolatile(), pointerType.isRestrict());
		}
		if (adaptedType instanceof ICPPTemplateTypeParameter) {
			ICPPTemplateArgument replacedType = templateParameterMap
					.getArgument((ICPPTemplateTypeParameter) adaptedType);
	...
	public void setImpliedObjectType(IType impliedObjectType) {
		...
		if (nonDeferredImpliedObjectType != null) {
			...
			if (impliedObjectType instanceof ITypeContainer) {
				IType ultimateType = SemanticUtil.getUltimateType(impliedObjectType, false);
				if (ultimateType instanceof ICPPUnknownBinding) {
					replacedImpliedObjectType = SemanticUtil.replaceNestedType((ITypeContainer) impliedObjectType,
							nonDeferredImpliedObjectType);
				}
			}
		}
	...
	public static LookupData createLookupData(IASTName name, ICPPTemplateParameterMap templateParameterMap,
			IType impliedObjectType) {
