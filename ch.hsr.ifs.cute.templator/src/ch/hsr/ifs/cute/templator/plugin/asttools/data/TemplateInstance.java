...
import ch.hsr.ifs.cute.templator.plugin.asttools.resolving.PostResolver;
...
public class TemplateInstance extends AbstractTemplateInstance {
	...
	protected TemplateInstance(UnresolvedNameInfo unresolvedName, ICPPASTTemplateDeclaration templateDefinition,
	...
	protected static TemplateInstance __create(UnresolvedNameInfo unresolvedName, AbstractResolvedNameInfo parent,
		...
		if (unresolvedName.getType() == NameTypeKind.FUNCTION_TEMPLATE
				|| unresolvedName.getType() == NameTypeKind.METHOD) {
			IASTFunctionDefinition functionDefinition = analyzer.getFunctionDefinition(resolvedTemplateInstance);
			templateDeclaration = ASTTools.findFirstAncestorByType(functionDefinition, ICPPASTTemplateDeclaration.class,
					2);

		} else if ((unresolvedName.getType() == NameTypeKind.CLASS_TEMPLATE
				|| unresolvedName.getType() == NameTypeKind.METHOD_TEMPLATE)
				&& resolvedTemplateInstance instanceof ICPPTemplateInstance) {
