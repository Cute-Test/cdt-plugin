...
In addition, `Student Project Workflow`_ describes the specifics/details that are of importance to students working on the CUTE CDT Plug-in as a term-project or bachelors thesis.
...
Getting started with plug-in development for Eclipse CDT can be scary at first.
...
**Use it!**.
ILTIS is included in the target platform definition of this plug-in, so there is nothing to set up.

### Use A Clean Eclipse Installation

To build the plug-ins, you will need either Eclipse PDE or Eclipse for Committers.
