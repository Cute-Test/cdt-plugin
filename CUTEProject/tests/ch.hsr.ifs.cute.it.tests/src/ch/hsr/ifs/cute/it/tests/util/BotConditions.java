...
public final class BotConditions {
     ...
     * @return
     */
    public static ViewTextContains viewTextContains(SWTBotView view, Supplier<AbstractSWTBotControl<? extends Control>> textAccessor,
            String pattern) {
        view.setFocus();
    ...
    private static final class IndexerIsDone extends DefaultCondition {
        ...
        public boolean test() throws Exception {
            ...
            return indexManager.isProjectIndexed(fProject) && indexManager.isIndexerIdle() && !indexManager.isIndexerSetupPostponed(fProject) &&
    ...
    private static final class JobExists extends DefaultCondition {
        ...
        @Override
        public boolean test() throws Exception {
            return Arrays.stream(Job.getJobManager().find(null)).anyMatch(job -> job.getName().equals(fJobName));
        }
