...
public class LaunchEnvironmentVariables {
   ...
   public static void apply(ILaunchConfigurationWorkingCopy wc, ICProject project) throws CoreException {
   ...
   private static void setMacDYLD_LIBRARY_PATH(ILaunchConfigurationWorkingCopy wc, ICProject project) throws CoreException {
      setPathEnvironmentVariable(wc, project, "DYLD_LIBRARY_PATH");
   }

   private static void setLinuxLD_LIBRARY_PATH(ILaunchConfigurationWorkingCopy wc, ICProject project) throws CoreException {
      setPathEnvironmentVariable(wc, project, "LD_LIBRARY_PATH");
   }

   private static void setWin32PATH(ILaunchConfigurationWorkingCopy wc, ICProject project) throws CoreException {
      setPathEnvironmentVariable(wc, project, "PATH");
   }

   //caveat:not for generic environment variable
   @SuppressWarnings({ "unchecked", "rawtypes" })
   private static void setPathEnvironmentVariable(ILaunchConfigurationWorkingCopy wc, ICProject project, String environmentVariableName)
      ...
      if (!path.equals(EMPTY_STRING)) if (!(path.charAt(path.length() - 1) + EMPTY_STRING).equals(pathSeparator)) path += pathSeparator;
   ...
   private static String generateLibPath(IProject[] libProject, String pathSeparator) {
      ...
      for (IProject element : libProject) {
         IManagedBuildInfo info = ManagedBuildManager.getBuildInfo(element);
         if (info != null) {
            ...
            for (ICOutputEntry outputEntry : dirs) {
               ...
               if (location.segmentCount() == 0) {
                  parameter = element.getFullPath();
               } else {
                  parameter = element.getFolder(location).getFullPath();
               }
